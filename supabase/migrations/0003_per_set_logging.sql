...
alter table public.program_exercises
  add column if not exists logging_mode text not null default 'weighted_sets'
    check (logging_mode in ('weighted_sets', 'bodyweight_sets', 'completion_only', 'variant'));
...
create index if not exists exercise_set_logs_log_idx
  on public.exercise_set_logs (exercise_log_id, set_number);
...
insert into public.exercise_set_logs (exercise_log_id, set_number, weight_lbs, reps, assisted_band)
select el.id, 1, el.weight_lbs, el.reps, false
  from public.exercise_logs el
 where (el.weight_lbs is not null or el.reps is not null)
   and not exists (
     select 1 from public.exercise_set_logs esl where esl.exercise_log_id = el.id
   );
